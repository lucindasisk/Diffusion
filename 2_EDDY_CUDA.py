...
subject_csv = read_csv(
    home + '/scripts/shapes/mri/dwi/shapes_dwi_subjList_11.15.2019.txt', sep=' ', header=None)
...
eddy_flow.connect([(infosource, sf, [('subject_id', 'subject_id')]),
                   ...
                   # Save b0 mask
                   (stripb0, datasink, [
                    ('mask_file', '3_Eddy_Corrected.@par')]),
                   # Run Eddy correction
                   (sf, eddy, [('dti', 'in_file'),
                               ('bval', 'in_bval'),
                               ('bvec', 'in_bvec'),
                   ...
                   (resample, datasink, [
                    ('resampled_file', '3_Eddy_Corrected.@par.@par.@par.@par.@par.@par.@par.@par.@par.@par.@par')])
                   ])
